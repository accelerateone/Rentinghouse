...
<head>
    ...
    <title>${siteName!""}|房屋分间信息管理-编辑房屋分间信息</title>
        ...
        <main class="lyear-layout-content">
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header"><h4>编辑房屋分间信息</h4></div>
                            <div class="card-body">
                                <form action="add" id="user-add-form" method="post" class="row">
                                    <input type="hidden" name="id" value="${houseDivisionInfo.id}">
                                ...
                                <#if housingEstate?size gt 0>
                                    <#list housingEstate as item>
                                         <option value="${item.id}"
                                                 <#if item.id==houseDivisionInfo.housingEstate.id>selected="selected"</#if> >
                                             ${item.name}
                                         </option>
                                    </#list>
                                </#if>
                            ...
                            <#if houseInfo??>
                                <#if houseInfo?size gt 0>
                                    <#list houseInfo as item>
                                         <option value="${item.id}"
                                                 <#if item.id==houseDivisionInfo.houseInfo.id>selected="selected"</#if> >
                                    ...
                                    <div class="input-group m-b-10">
                                        ...
                                        <input type="text" class="form-control required" id="name" name="name"
                                               value="${houseDivisionInfo.name}" placeholder="请输入分间名" tips="请填写分间名"/>
                                    </div>
                                    <div class="input-group m-b-10">
                                        <span class="input-group-addon">分间面积：</span>
                                        <input type="number" class="form-control required" id="divisionArea"
                                               name="divisionArea" value="${houseDivisionInfo.divisionArea}"
                                               placeholder="请输入分间面积（单位㎡，只需填写数字，不需填写单位名称）" tips="请填写分间面积"/>
                                    </div>
                                    <div class="input-group m-b-10">
                                        <span class="input-group-addon">分间朝向情况：</span>
                                        <input type="text" class="form-control required" id="orientation"
                                               name="orientation" value="${houseDivisionInfo.orientation}"
                                               placeholder="请输入分间朝向情况" tips="请填写分间朝向情况"/>
                                    </div>
                                    <div class="input-group m-b-10">
                                        <span class="input-group-addon">分间窗户情况：</span>
                                        <select name="houseDivisionWindowEnum" class="form-control" id="houseDivisionWindowEnum">
                                        <#list houseDivisionWindowEnums as item>
                                            <option value="${item}"
                                                    <#if houseDivisionInfo.houseDivisionWindowEnum.getCode()==item.getCode()>selected="selected"</#if> >
                                                ${item.getName()}
                                            </option>
                                        </#list>
                                        </select>
