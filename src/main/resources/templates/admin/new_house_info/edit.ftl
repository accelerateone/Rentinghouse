...
<head>
    ...
    <title>${siteName!""}|楼盘管理-编辑楼盘信息</title>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form id="new-house-info-add-form" class="row">
                                    <input type="hidden" name="id" value="${newHouseInfo.id}">
                                                ...
                                                <li class="col-xs-4 col-sm-3 col-md-2">
                                                    <figure>
                                                        <#if newHouseInfo.firstFigurePath?length gt 0>
                                                            <img src="/photo/view?filename=${newHouseInfo.firstFigurePath}"
                                                                 id="show-picture-img">
                                                        <#else>
                                                            <img src="/admin/images/default-head.jpg"
                                                                 id="show-picture-img" alt="默认效果图">
                                                        </#if>
                                                    </figure>
                                                </li>
                                                <input type="hidden" name="firstFigurePath" id="headPic"
                                                       value="${newHouseInfo.firstFigurePath}">
                                    ...
                                    <div class="input-group m-b-10">
                                        ...
                                        <input placeholder="请输入详细地址(无需省市区)" tips="请填写详细地址(无需省市区)"
                                               class="form-control" id="detailedAddress"
                                               value="${newHouseInfo.detailedAddress}" name="detailedAddress"/>
                                    </div>

                                    <div class="input-group m-b-10">
                                        <span class="input-group-addon">楼盘名称</span>
                                        <input type="text" class="form-control required" id="name" name="name"
                                               value="${newHouseInfo.name}" placeholder="请输入楼盘名称" tips="请填写楼盘名称"/>
                                    </div>
                                    ...
                                    <div class="input-group m-b-10">
                                        ...
                                        <select name="housingCharacteristicsList" class="form-control selectpicker"
                                            ...
                                            <#list housingCharacteristicss as housingCharacteristics>
                                                <option <#list newHouseInfo.housingCharacteristicsList as housingCharacteristicsList> <#if housingCharacteristics.id == housingCharacteristicsList.id>selected</#if> </#list>
                                                        value="${housingCharacteristics.id}">
                                        ...
                                        <select name="decorateState" class="form-control" id="automobileType">
                                            <#if decorateStates??>
                                                <#list decorateStates as decorateState>
                                                    <option <#if newHouseInfo.decorateState.getCode() == decorateState.getCode()>selected</#if>
                                                            value="${decorateState}"
                                            ...
                                            <#if propertyTypes??>
                                                <#list propertyTypes as propertyType>
                                                    <option value="${propertyType}"
                                                            <#if newHouseInfo.propertyType.getCode() == propertyType.getCode()>selected</#if>
                                            ...
                                            <#if sellingTheStates??>
                                                <#list sellingTheStates as sellingTheState>
                                                    <option value="${sellingTheState}"
                                                            <#if newHouseInfo.sellingTheState.getCode() == sellingTheState.getCode()>selected</#if>
                                    ...
                                    <div class="input-group m-b-10">
                                        ...
                                        <input type="number" class="form-control required" id="builtUpArea"
                                               ...
                                               name="builtUpArea" tips="请填写建成面积(㎡)" value="${newHouseInfo.builtUpArea}"
                                               placeholder="请输入建成面积(㎡)"/>
                                    </div>
                                    ...
                                    <div class="input-group m-b-10">
                                        ...
                                        <select name="user" class="form-control selectpicker" multiple
                                                data-live-search="true" id="user">
                                            <#list consultants as consultant>
                                                <option <#if newHouseInfo.user.id == consultant.id>selected</#if>
                                                        value="${consultant.id}">
                                                    顾问名称:${consultant.username}
                                                </option>
                                            </#list>
                                        </select>
                                    </div>
                                    ...
                                    <div class="input-group m-b-10">
                                        ...
                                        <select name="users" class="form-control selectpicker"
                                                multiple data-live-search="true" id="automobileInformation">
                                            <#list agents as agent>
                                                <option <#list newHouseInfo.users as user><#if user.id == agent.id>selected</#if></#list>
                                                        value="${agent.id}">
                                                    经纪人名称:${agent.username}
                                                </option>
                                            </#list>
                                        </select>
                                    </div>
                                    ...
                                    <div class="input-group m-b-10">
                                        ...
                                        <input class="form-control js-datepicker"
                                               value="${newHouseInfo.latestOpeningHours}" type="text"
                                               id="LatestOpeningHours" name="LatestOpeningHours"
                                            ...
                                            <#if openingTimes??>
                                                <#list openingTimes as openingTime>
                                                    <option value="${openingTime}"
                                                            <#if newHouseInfo.openingTime.getCode() == openingTime.getCode()>selected</#if>
                                    ...
                                    <div class="input-group m-b-10">
                                        ...
                                        <input type="number" class="form-control required" id="quarterlyPayment"
                                               onkeyup="inputDouble(this)"
                                               name="averagePrice" value="${newHouseInfo.averagePrice}"
                                               tips="请填写均价(单价)(单位元)" placeholder="请输入均价(单价)(单位元)"/>
                                    </div>
                                    ...
                                    <div class="input-group m-b-10">
                                        ...
                                        <input type="text" class="form-control required" id="longitudeAndIatitude" name="longitudeAndIatitude"
                                               value="${newHouseInfo.longitudeAndIatitude}" placeholder="请填写经纬度（不清楚可以去下方链接自行拾取）" tips="请填写经纬度"/>
                                    </div>
                                    <div class="input-group m-b-10">
...
<script>
    ...
    $(function () {
        ...
        $("#distpicker").distpicker({
            province: '${newHouseInfo.province}',
            city: '${newHouseInfo.city}',
            district: '${newHouseInfo.district}'
        });
    })
</script>
...
<script>
    ...
    var pathConfigArr = new Array();
    <#if newHouseInfo.effectPictureItems??>
    <#list newHouseInfo.effectPictureItems as effectPicture>
    pathArr.push("/photo/view?filename=${effectPicture.effectPicturePath}");
    pathConfigArr.push({
        key: "${effectPicture.id}",
        caption: "${effectPicture.effectPicturePath}"
    })
    resultPathData.push('${effectPicture.effectPicturePath}');
    resultProviewIdData.push('${effectPicture.id}')
    </#list>
    </#if>
    ...
    $("#fileInput").fileinput({
        ...
        initialPreview: pathArr,
        initialPreviewConfig: pathConfigArr
    ...
    $(document).ready(function () {
        $('.kv-file-remove.btn.btn-sm.btn-kv.btn-default.btn-outline-secondary').on('click', function () {
            ...
            if (dataKey != "undefined") {
                var dataKeyIndex = resultProviewIdData.indexOf(dataKey);
                resultPathData.splice(dataKeyIndex, 1, "null");
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $("#add-form-submit-btn").click(function () {
            ...
            if (!checkForm("new-house-info-add-form")) {
                return;
            }
                ...
                success: function (data) {
                    ...
                    if (data.code == 0) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data.code == 0) {
                                    showSuccessMsg('楼盘信息编辑成功!', function () {
